...
- **情绪记录**：组合检索（备注关键词、标签多选且全部命中、心情指数区间、日期单独限定），可在记录日期/心情指数两种排序间切换；筛选条件同步到网址，刷新或重新打开链接仍保持结果，清空后恢复全部记录；网址含无效区间或未知标签时自动回退到可用默认值并说明原因。`MoodSelector` 在 Dashboard 和 Moods 页面共享。
